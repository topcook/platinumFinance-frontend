...
import { FTM_TICKER, PLAT_TICKER, SPLAT_TICKER, ZAPPER_ROUTER_ADDR } from '../utils/constants';
...
function useApproveZapper(zappingToken: string): [ApprovalState, () => Promise<void>] {
  ...
  let token: ERC20;
  if (zappingToken === FTM_TICKER) token = platFinance.FTM;
  else if (zappingToken === PLAT_TICKER) token = platFinance.PLAT;
  else if (zappingToken === SPLAT_TICKER) token = platFinance.SPLAT;
  ...
  const approvalState: ApprovalState = useMemo(() => {
    ...
    if (token === platFinance.FTM) return ApprovalState.APPROVED;
  ...
  }, [currentAllowance, pendingApproval, token, platFinance]);
